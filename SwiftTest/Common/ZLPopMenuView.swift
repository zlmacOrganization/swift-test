...
protocol ZLPopMenuViewDelegate: AnyObject {
    func ZLPopMenuViewDidSelectAt(index: Int)
}

public enum ZLPopMenuViewConfigure {
...
public class ZLPopMenuView: UIView {
    
    //delegate
    weak var delegate: ZLPopMenuViewDelegate?
    //block
    public var didSelectMenuBlock: ((Int) -> Void)?
    ...
    //字体大小等
    private var popMenuTextFont: UIFont = UIFont.systemFont(ofSize: 14)
    //菜单高度
    ...
    static let cellID: String = "ZLPopMenuViewCellID"
    private var myFrame: CGRect = .zero
 ...
 ///
 /// - Parameters:
 ///   - menuWidth: 菜单宽度，默认150
 ///   - relyOnView: 依赖视图
 ///   - datas: 数据源，icon允许传空，数据源没数据，不会显示菜单
 ///   - configure: 配置信息，可不传
    init(menuWidth: CGFloat = 150,
         relyOnView: UIView,
         datas: [(icon: String, title: String)],
         configures: [ZLPopMenuViewConfigure] = []) {
        ...
        let relyFrame = relyOnView.convert(relyOnView.bounds, to: getKeyWindow())
        myFrame = CGRect(x: 0, y: 0, width: menuWidth, height: popMenuItemHeight*CGFloat(popData.count))
        myFrame.size.height = min(popMenuItemHeight*6, myFrame.height)
        myFrame.size.width = min(KScrW-popMenuMargin*2, myFrame.width)
        
        //设置箭头，与屏幕间隔10
        arrowPoint = CGPoint(x: relyFrame.midX, y: relyFrame.maxY + 5)
    ...
    private func initViews() {
        ...
        tableView.register(ZLPopMenuViewCell.self, forCellReuseIdentifier: ZLPopMenuView.cellID)
    ...
    private func getArrowPoints() -> (CGPoint, CGPoint, CGPoint, CGPoint, Bool) {
...
extension ZLPopMenuView : UITableViewDataSource, UITableViewDelegate{
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if popData.count>indexPath.row {
            let cell = tableView.dequeueReusableCell(withIdentifier: ZLPopMenuView.cellID) as! ZLPopMenuViewCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if self.delegate != nil{
            self.delegate?.ZLPopMenuViewDidSelectAt(index: indexPath.row)
        }
...
class ZLPopMenuViewCell: UITableViewCell {
    ...
    func setConfig(_txtColor: UIColor, _lineColor: UIColor, _txtFont: UIFont, _iconLeft: CGFloat) {
        ...
        lblTitle.textColor = _txtColor
        lblTitle.font = _txtFont
    }
