//
//  CollectionAlignmentFlowLayout.swift
//  SwiftTest
//
...
class CollectionAlignmentFlowLayout: UICollectionViewFlowLayout {
    //两个Cell之间的距离
    private var itemSpacing: CGFloat = 5
    private var lineSpacing: CGFloat = 5
    
    //cell对齐方式
    private var cellType : AlignType = .center
    ...
    convenience init(_ cellType: AlignType, _ itemSpacing: CGFloat = 5, _ lineSpacing: CGFloat = 5) {
        self.init()
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        let layoutAttributes : [UICollectionViewLayoutAttributes] = super.layoutAttributesForElements(in: rect) ?? [UICollectionViewLayoutAttributes]()
    ...
    func setCellFrame(with layoutAttributes : [UICollectionViewLayoutAttributes]) {
        ...
        case .left:
            ...
            for attributes in layoutAttributes {
                ...
                nowWidth += nowFrame.size.width + self.itemSpacing
            }
            
        case .center:
            nowWidth = (self.collectionView!.frame.size.width - sumCellWidth - (CGFloat(layoutAttributes.count - 1) * itemSpacing)) / 2
            for attributes in layoutAttributes {
                var nowFrame = attributes.frame
                nowFrame.origin.x = nowWidth
                attributes.frame = nowFrame
