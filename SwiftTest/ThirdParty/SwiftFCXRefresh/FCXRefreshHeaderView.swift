...
open class FCXRefreshHeaderView: FCXRefreshBaseView {
    ...
    open override func scrollViewContentOffsetDidChange(scrollView: UIScrollView) {
        ...
        let happenOffsetY = scrollViewOriginalEdgeInsets.top as CGFloat
        
        if scrollView.contentOffset.y > -happenOffsetY {
            //向上滚动到看不见头部控件，直接返回
            return
        }

        if scrollView.isDragging {//正在拖拽
            if scrollView.contentOffset.y + happenOffsetY < -hangingHeight {//大于偏移量，转为pulling
                state = .pulling
        ...
        if pullingPercentHandler != nil {
            let offsetHeight = -scrollView.contentOffset.y - happenOffsetY
