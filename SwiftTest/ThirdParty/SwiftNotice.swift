...
class SwiftNotice: NSObject {
    ...
    static let rv = getKeyWindow()?.subviews.first as UIView?
    ...
    static var degree: Double {
        get {
            return [0, 0, 180, 270, 90][getKeyWindow()?.windowScene?.interfaceOrientation.hashValue ?? 0] as Double
        }
    }
    ...
    static func noticeOnStatusBar(_ text: String, autoClear: Bool, autoClearTime: Int) -> UIWindow{
        ...
        if let version = Double(UIDevice.current.systemVersion),
            ...
            let x = [0, screenWidth/2, screenWidth/2, 10, screenWidth-10][getKeyWindow()?.windowScene?.interfaceOrientation.hashValue ?? 0] as CGFloat
            let y = [0, 10, screenHeight-10, screenHeight/2, screenHeight/2][getKeyWindow()?.windowScene?.interfaceOrientation.hashValue ?? 0] as CGFloat
    ...
    static func getRealCenter() -> CGPoint {
        if getKeyWindow()?.windowScene?.interfaceOrientation.hashValue ?? 0 >= 3 {
